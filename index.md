...
An objection deserves a hearing here, because it is the strongest one against everything above: is advanced nuclear not just another China-mineral dependency, the way robotics is? The answer is no, and the reason is a distinction the phrase *critical minerals* usually blurs. Robots ride on a single chokepoint — NdFeB permanent magnets, whose refining and manufacture China controls to about 90%, with export controls already active and no near-term workaround [Payne '26]. Reactors barely touch it. Rare-earth magnets appear in an SMR only in pumps, drives, and compact generators: a 100-GW fleet would need roughly 2,000 to 5,000 tonnes of them, a rounding error against China's output of about 300,000 tonnes a year.

*(figure: Robots vs reactors: where the mineral chokepoints actually are — The map, not the thesis. The left panel and the chokepoint ranking are ESTABLISHED (ledger C37): robotics depends on one chokepoint held ~90% by one country, while reactor exposure is spread across HALEU (Russia, fixable), hafnium and zirconium (allies), graphite (China, and only for high-temperature designs), and beryllium (the US itself) — rare-earth magnets are a rounding error. The 10–15-year buildout clock is the separate OPEN-CAVEATED claim C38. This figure asserts the supply map, not the maker thesis of C36.)*

The real bill sits elsewhere, and it is worth ranking honestly [CATF '25]. First and largest is HALEU: Russia was the sole commercial supplier as of 2024, and a 100-GW fleet would need 500 to 1,000 tonnes a year — fifty to a hundred times current Western capacity — which is precisely the gap Centrus and claim `C35` are aimed at. Second is hafnium, the thinnest non-fuel input: a 100-GW fleet would need five to ten times current global production, and supply is inelastic because hafnium is a byproduct of zirconium refining; France supplies about half of it, which makes the holder an ally [USGS-derived]. Third is nuclear-grade zirconium — the ore is abundant, from Australia and South Africa, but nuclear-grade processing sits in a handful of facilities in China, the US, and France, and a 100-GW fleet would need three to four times today's output. Fourth is nuclear graphite, the one genuine China exposure — and only for high-temperature designs such as Kairos and X-energy. Fifth is beryllium, where the US produces about half of world supply, so for salt designs the leverage runs toward Washington rather than away from it.

The honest caveat is the clock. Standing up mineral and fuel supply chains at SMR scale is estimated at ten to fifteen years, which means the buildout would have to begin roughly now to serve a 2035–2040 fleet; hafnium is expected to stay tight at scale even with France onside; and China is building HALEU conversion and enrichment capacity while explicitly modelling its rare-earths playbook for uranium [minerals geopolitics '26]. That last point is the one to watch: the fuel window would close on the same clock as the reactor window. This is claim `C38`, **OPEN-CAVEATED**. **Signpost:** US and allied HALEU capacity ≥25 t/yr by 2030 — the fleet-scale aggregate that the Centrus Piketon milestone of `C36` sits inside, rather than repeats.

*Under today's supply map, the US cannot build a robot without Beijing's consent — but it can build a reactor without it.*

That is a claim about the present map, not a permanent condition: chokepoints move, and the clock above is exactly the mechanism by which this one could. But it does dispose of the strongest objection to what follows — that the maker question is moot because the minerals are already lost.
...
- **Payne '26** — Payne Institute for Public Policy (2026). Materials analysis of advanced reactors: rare-earth magnets appear only in pumps, drives, and compact generators, so a 100-GW SMR fleet needs roughly 2,000–5,000 t of magnets — negligible against China's ~300,000 t/yr output. The binding inputs are fuel and specialty metals, not rare earths. *Payne Institute, advanced-nuclear materials analysis (2026), payneinstitute.mines.edu*

- **CATF '25** — Clean Air Task Force (2025). Advanced-nuclear supply-chain assessment ranking the real bottlenecks — HALEU first, then hafnium and nuclear-grade zirconium, with nuclear graphite an exposure only for high-temperature designs — and sizing each against a 100-GW fleet. *Clean Air Task Force, advanced nuclear supply chains (2025), catf.us*

- **USGS-derived** — Critical/strategic metals production data (USGS-derived). Production-share data behind the chokepoint ranking: France supplies roughly half of global hafnium (a byproduct of zirconium refining, hence inelastic), zircon ore comes mainly from Australia and South Africa, and the US produces about half of world beryllium. *USGS Mineral Commodity Summaries and derived critical-metals compilations*

- **minerals geopolitics '26** — Minerals-geopolitics literature (2026). On the clock: standing up SMR-scale mineral and fuel supply chains is estimated at 10–15 years, and China is building HALEU conversion and enrichment capacity while explicitly modelling its rare-earths playbook for uranium — so the fuel window closes on the same timetable as the reactor window. *ScienceDirect minerals-geopolitics analyses (2026) and China-enrichment reporting*
